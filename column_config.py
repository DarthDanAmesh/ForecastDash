...
STANDARD_COLUMNS = {
    ...
    'act_gds_issue_date': 'act_gds_issue_date',
    'customer_reference': 'customer_reference',
}

COLUMN_ALIASES = {
    'date': [
        'date', 'created on', 'created_on', 'timestamp', 'order date', 'document date', 
        'customer ref. date'
    ],
    'demand': [
        'quantity', 'delivery quantity', 'sales', 'orders', 'value', 'volume', 
        'deliv.value lips doc.curr.', 'qty', 'delivery_quantity'
    ],
    'country': [
        'country', 'country_code', 'ship_to_country', 'customer country', 'region', 
        'country key ship-to'
    ],
    'material': [
    ...
    'delivery_date': [
        'delivery date', 'act. gds issue date', 'shipment date', 'ship_date', 
        'act_gds_issue_date'
    ],
    ...
    'delivery_quantity': [
        'delivery quantity', 'qty', 'deliv qty', 'delivery'
    ],
    'act_gds_issue_date': [
...
def standardize_column_names(df):
    ...
    for std_key, aliases in COLUMN_ALIASES.items():
        for alias in [a.lower() for a in aliases]:
            if alias in df.columns and std_key not in column_mapping.values():
                column_mapping[alias] = std_key
...
def get_standard_name(original_name):
    ...
    Returns:
        str: Standardized column name or original name if no mapping exists.
    """
